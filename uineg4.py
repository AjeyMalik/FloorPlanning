...
import numpy as np
from neg4 import FloorPlan  # Import from your original file
import json
...
class CADApp:
    ...
    def setup_ui(self):
        ...
        ttk.Button(control_frame, text="Draw Area", command=self.draw_area).pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text="Finalize", command=self.is_finalize2).pack(side=tk.LEFT, padx=5)

        
        # ttk.Button(control_frame, text="Edit Selected", command=self.edit_region).pack(side=tk.LEFT, padx=2)
        # ttk.Button(control_frame, text="Remove Selected", command=self.remove_region).pack(side=tk.LEFT, padx=2)
        # ttk.Button(control_frame, text="Clear All", command=self.clear_regions).pack(side=tk.LEFT, padx=2)
            ...
            if closest_point and (closest_point[0], closest_point[1]) != self.last_valid_point:
                ...
                if self.is_aligned(self.last_valid_point, (closest_point[0], closest_point[1])):
                    ...
                    self.canvas.create_text(mid_x, mid_y - 10, text=f"{distance:.0f} units",
                                        fill='green', tags="hover_distance", font=('Arial', 8))
            else:
                if self.hover_line:
                    self.canvas.delete(self.hover_line)
                    self.canvas.delete("hover_distance")
    ...
    def on_click2(self, event):
            ...
            #  to check before is_closedd_shape so that the final_area list can append the area_coords correctly
                ...
                print(f"Clicked: Grid({grid_x}, {grid_y}) = Units({grid_x * self.unit_spacing}, {grid_y * self.unit_spacing})")
    ...
    def draw_line_with_distance2(self, grid_x, grid_y, pixel_x, pixel_y):
            ...
            line = self.canvas.create_line(
                ...
                fill='green', width=2, tags="drawn_line"
            )
            ...
            distance_label = self.canvas.create_text(
                mid_x + 10, mid_y - 10, text=f"{distance:.0f}",
                fill='green', font=('Arial', 9, 'bold'), tags="distance_label"
            )
    ...
    def is_finalize2(self):
        ...
        # for a, b in self.area_coords:
        #     scaled_point = (a * self.unit_spacing, b * self.unit_spacing)
        #     self.scaled_area_coordinates.append(scaled_point)

        # self.scaled_area_coordinates = [
        #     (a - self.mini_x, b - self.mini_y) for a, b in self.scaled_area_coordinates
        # ]

        # for a,b in self.scaled_area_coordinates:
        #     print(a,b)
        ...
        for area_coords in self.final_area:
            ...
            # Step 2: Shift points by mini_x and mini_y
            

            if(self.unit_spacing_prev>self.unit_spacing):
                shifted_area = scaled_area
            else:
                shifted_area = [(a - self.mini_x, b - self.mini_y) for a, b in scaled_area]
            ...
            # for i, area in enumerate(self.scaled_final_area):
            #     print(f"Scaled Area {i+1}:")
            #     for a, b in area:
            #         print(a, b)


        print("the length of is ",len(self.final_area))
        print(self.scaled_final_area)
    ...
    def decompose_into_rectangles(self):
        ...
        excluding the areas defined by self.scaled_final_area (list of polygons).
        Stores the regions in self.regions as dictionaries with x, y, width, and height.
        """
        self.regions = []
        
        # Debug prints
        print("Main boundary coordinates:", self.scaled_coordinates)
        if hasattr(self, 'scaled_final_area') and self.scaled_final_area:
            print(f"Number of areas to exclude: {len(self.scaled_final_area)}")
            for i, area in enumerate(self.scaled_final_area):
                print(f"Area {i+1} to exclude coordinates:", area)
        else:
            print("No areas to exclude")
        
        # Get unique x-coordinates and y-coordinates from ALL vertices (boundary and all excluded areas), sorted
        all_coords = list(self.scaled_coordinates)
        if hasattr(self, 'scaled_final_area') and self.scaled_final_area:
            for area_coords in self.scaled_final_area:
                if area_coords:  # Check if area_coords is not empty
                    all_coords.extend(area_coords)
        
        x_coords = sorted(set(x for x, y in all_coords))
        y_coords = sorted(set(y for x, y in all_coords))
        ...
        for i in range(len(x_coords) - 1):
            ...
            for j in range(len(y_coords) - 1):
                ...
                # Test if the center of this rectangular cell is inside the main polygon
                test_x = (x_left + x_right) / 2
                test_y = (y_bottom + y_top) / 2
                
                # Check if point is inside main boundary
                inside_main = point_in_polygon(test_x, test_y, self.scaled_coordinates)
                
                # Check if point is inside any of the areas to exclude
                inside_exclude = False
                if hasattr(self, 'scaled_final_area') and self.scaled_final_area:
                    for area_coords in self.scaled_final_area:
                        if area_coords and point_in_polygon(test_x, test_y, area_coords):
                            inside_exclude = True
                            break  # No need to check other areas once we find one
                
                # Debug prints
                print(f"Testing cell: x=[{x_left},{x_right}], y=[{y_bottom},{y_top}], center=({test_x},{test_y})")
                print(f"  Inside main: {inside_main}, Inside exclude: {inside_exclude}")
                
                # Include this cell only if it's inside main boundary but NOT inside any exclude area
                if inside_main and not inside_exclude:
                    y_ranges.append((y_bottom, y_top))
                    print(f"  -> Added to y_ranges")
                ...
                for y_start, y_end in merged_ranges:
                    ...
                    if height > 0 and width > 0:
                        self.regions.append({
                            ...
                            'y': y_start,         # Bottom edge y-coordinate
                            'width': width,       # Width of rectangle
                            'height': height      # Height of rectangle
                        })
        
        # Normalize coordinates - flip y-coordinates to match original orientation
        if self.regions:
            min_x = min(region['x'] for region in self.regions)
            max_y = max(region['y'] + region['height'] for region in self.regions)
            min_y = min(region['y'] for region in self.regions)
            
            for region in self.regions:
                region['x'] = region['x'] - min_x
                # Flip y-coordinate to match original polygon orientation
                region['y'] = max_y - (region['y'] + region['height'])
        
        print(f"Total regions generated: {len(self.regions)}")
        if self.regions:
